...
def imshow_incorrect(img, right, label, number, type):
    ...
    plt.imsave(os.path.join('drive/Shareddrives/VGU_Thesis/runs/Wrong_label', 'image_show_' + str(right) + '#' + str(label) + '_' + str(number) + '.png'), np.transpose(npimg, (1, 2, 0)))

def imshow_correct(img, label, number, type):
    img = img / 2 + 0.5  # unnormalize
    npimg = img[0].numpy()
    # print('Image: ')
    # print(label)
    ...
    # print(type)
    plt.imshow(np.transpose(npimg, (1, 2, 0)))
    plt.imsave(os.path.join('drive/Shareddrives/VGU_Thesis/runs/Right_label', 'image_show_' + str(label) + '_' + str(number)  + '.png'), np.transpose(npimg, (1, 2, 0)))

def eval_dataset(type, model, data_loader, num_classes, device, samples=1, calc_reconstruction=False, autoregression=False):
    """
    ...
    save_path = 'drive/Shareddrives/VGU_Thesis/runs/' + \
                '_eval_dataset_' + str(args.var_samples) + '_latent_dim_' + str(args.var_latent_dim)
        ...
        for j, (inputs, classes) in enumerate(data_loader):
            ...
            for i in range(inputs.size(0)):
                ...
                if classes[i].item() != idx:
                    ...
                    imshow_incorrect(images, idx, classes[i].item(), incorrectly_identified, type)
                else:
                    ...
                    imshow_correct(images, classes[i].item(), correctly_identified, type)
            visualize_image_grid(images, writer, count, '_dataset_snapshot', save_path)
    ...
    dic = {"accuracy": acc, "encoded_mus_correct": encoded_mus_correct, "encoded_mus_false": encoded_mus_false,
...
def eval_openset_dataset(model, data_loader, num_classes, device, samples=1,
    ...
    args = parser.parse_args()
    save_path = 'drive/Shareddrives/VGU_Thesis/runs/' + \
                '_eval_open_dataset_' + str(args.var_samples) + '_latent_dim_' + str(args.var_latent_dim)
    writer = SummaryWriter(save_path)
    count = 0
    # evaluate the encoder and classifier and store results in corresponding lists according to predicted class.
    # Prediction mean confidence and uncertainty is also obtained if amount of latent samples is greater than one.
        ...
        for j, (inputs, classes) in enumerate(data_loader):
            ...
            for i in range(inputs.size(0)):
                ...
                if calc_reconstruction:
                    recon_loss_mus.append(recon_loss_mu[i].item())
                    recon_loss_sigmas.append(recon_loss_sigma[i].item())
            visualize_image_grid(images, writer, count, '_open_dataset_snapshot', save_path)
    ...
    dic = {"encoded_mus": encoded_mus, "encoded_sigmas": encoded_sigmas,
