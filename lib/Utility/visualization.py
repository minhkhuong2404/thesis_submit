...
def visualize_classification_uncertainty(data_mus, data_sigmas, other_data_dicts, other_data_mu_key,
    ...
    plt.scatter(data_mus, data_sigmas, label=data_name, s=75, c="yellow", alpha=1.0)
...
def visualize_classification_scores(data, other_data_dicts, dict_key, data_name, save_path):
    ...
    plt.figure(figsize=(20, 20))
    plt.hist(data, label=data_name, alpha=1.0, bins=20, color="yellow", histtype='bar', density=True)
    ...
    for other_data_name, other_data_dict in other_data_dicts.items():
        ...
        group_data.append(other_data)
        group_label.append(other_data_name)

    plt.hist(group_data, label=group_label, color=colors, alpha=0.5, bins=20, histtype='bar', density=True)
...
def visualize_weibull_outlier_probabilities(data_outlier_probs, other_data_outlier_probs_dict,
    ...
    plt.figure(figsize=(20, 20))
    plt.hist(data_outlier_probs, label=data_name, weights=data_weights, bins=50, color="yellow",
             alpha=1.0, edgecolor='white', linewidth=5, histtype='bar', density=True)
    ...
    for other_data_name, other_data_outlier_probs in other_data_outlier_probs_dict.items():
        ...
        plt.hist(other_data_outlier_probs, label=other_data_name, weights=other_data_weights,
                 bins=50, color=colors[c], alpha=0.5, edgecolor='white', linewidth=5, histtype='bar', density=True)
        c += 1
...
def visualize_openset_classification(data, other_data_dicts, dict_key, data_name,
    ...
    lw = 10
    plt.figure(figsize=(20, 20))
    plt.plot(thresholds, data, label=data_name, color="yellow", linestyle='solid', linewidth=lw)
