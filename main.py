...
def main():
    ...
    save_path = 'drive/Shareddrives/VGU_Thesis/runs/' + strftime("%Y-%m-%d_%H-%M-%S",
    ...
    epoch_multiplier = 1
    if args.incremental_data:
        np.save('task_order.npy', np.array([0, 1, 2, 3, 4]))
        ...
        if args.cross_dataset:
            ...
            if args.load_task_order:
                if os.path.isfile(args.load_task_order):
                    print("=> loading task order from '{}'".format(args.load_task_order))
                    task_order = np.load(args.load_task_order, allow_pickle=True).tolist()
                else:
