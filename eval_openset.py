...
def main():
    ...
    split_sets = torch.utils.data.random_split(dataset.valset,
                                               [int((1 - split_perc) * len(dataset.valset) + 1) ,
    ...
    dataset_eval_dict_train = eval_dataset("train", model, dataset.train_loader, dataset.num_classes, device,
    ...
    threshset_eval_dict = eval_dataset("threshold", model, dataset.threshset_loader, num_classes, device, samples=args.var_samples,
    ...
    dataset_eval_dict = eval_dataset("validation", model, dataset.val_loader, num_classes, device, samples=args.var_samples,
                                     calc_reconstruction=args.calc_reconstruction, autoregression=args.autoregression)
